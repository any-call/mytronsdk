...
import (
	...
	"github.com/any-call/mytronsdk/proto/core"
...
func GetPaddedParam(param []Param) ([]byte, error) {
	...
	for i, v := range values {
		switch val := v.(type) {
		case float64:
			values[i] = big.NewInt(int64(val))
		case int, int64:
			values[i] = big.NewInt(reflect.ValueOf(val).Int())
		}
	}

	//amount := big.NewInt(1_000_000) // 把 1e+06 转成 *big.Int
